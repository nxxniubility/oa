...
$(function(){
    ...
    $(document).on('click','.Offline',function(){
        ...
        if(data_id==''){
            var title = '确定要强制离线全体员工吗？';
        }else{
            var title = '确定要强制离线该员工吗？';
        };
        layer.confirm(title, {
            btn: ['确定','取消'] //按钮
        }, function(){
            var data = {
                system_user_id : data_id
            };
            common_ajax(data,"{:U('System/Personnel/SystemUserOffline')}", 'no');
        }, function(){});
    });
    //离职设置-前置判断
    $(document).on('click','.usertypeBtn',function(){
        ...
        layer.confirm('确定要设置离职？如该员工拥有客户，将被全部回库！', {
            btn: ['确定','取消'] //按钮
        }, function(){
