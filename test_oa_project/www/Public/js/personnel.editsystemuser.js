...
$(document).on('click','dd',function(){
    if(!$(this).parent().hasClass('clearfix')){
        if($(this).parent().attr('id')=='zone_li'){
            $('#zonename_title').hide();
        };
    ...
    $(document).on('click','.nsSubmit',function() {
        ...
        common_ajax2(data,'/SystemApi/SystemUser/editSystemUser','no',function(reflag) {
            if(reflag.code==0){
                ...
                layer.confirm(reflag.msg, {
                    btn: ['确定','取消'] //按钮
                }, function(){
                    common_ajax(data2,"/SystemApi/SystemUser/editUserZone",'no',function(reflag) {
                        window.location.href = "/System/Personnel/systemUserList";
                    });
                }, function(){
                    window.location.href = "/System/Personnel/systemUserList";
...
//获取职位列表
$('.nsMiddle').html(getLoding());
common_ajax2({system_user_id: $.getUrlParam('user_id')},'/SystemApi/SystemUser/getSystemUserInfo','no',function(redata){
    if(redata.data){
        layui.use('laytpl', function(){
            var laytpl = layui.laytpl;
            laytpl(demo.innerHTML).render(redata.data, function(result){
                $('.nsMiddle').html(result);
                //  几个时间初始化
