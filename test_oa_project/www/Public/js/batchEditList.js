...
$('#pcUploadBtn').on('click',function(){
	...
	$('#pcSure').on('click',function(){
		...
		var pc_pages_name = $(':input[name="selectCategory1"]:checked').parents('.clearfix').find('.theme').text();
		...
		if ( $(':input[name="selectCategory1"]:checked').length>0) {
			...
			$('#pcUploadBtn').next('span').html(pc_pages_name);
...
$('#mUploadBtn').on('click',function(){
	...
	$('#mSure').on('click',function(){
		...
		var m_pages_name = $(':input[name="selectCategory2"]:checked').parents('.clearfix').find('.theme').text();
		...
		if ( $(':input[name="selectCategory2"]:checked').length>0) {
			...
			$('#mUploadBtn').next('span').html(m_pages_name);
			layer.closeAll(); 		// 关闭
		}else{
			layer.msg('未选择模板',{icon:2});
		}
