...
(function() {
    ...
    $.fn.glDatePicker.defaults = {
        ...
        Todate:(function(num){
            ...
            str = num.split(" ");
            date = str[3] + "-";
            var count = this.strlen(month[str[1]]);
            var m = month[str[1]];
            if (count = 1) {
                m = "0" + month[str[1]];
            }
        ...
        glDatePicker.prototype = {
            ...
            render: function(renderCalback) {
                ...
                if( options.selectableYears ){
                    var startYear = options.selectableYears[0];
                    var endYear = options.selectableYears[1];
                }else{
                    var startYear = 10;
                    var endYear = 5;
                }
                var selectableYears = getSelectableList(todayVal.year - startYear, todayVal.year + endYear, options.selectableYears);
                ...
                for (var row = 0, cellIndex = 0; row < maxRow + 1; row++) {
                    for (var col = 0; col < maxCol; col++, cellIndex++) {
                            ...
                            if (!isSelectable ||
                                ...
                                cell
                                    ...
                                    .click(function(e) {
                                        ...
                                        //this day without current month
                        ...
                        //remove null cell
                        if (cell.hasClass("outday")) {
                            nullcell++;
                            if (nullcell <= 7 && cellIndex <= 14) {
                                // Add cell to calendar 
                                calendar.append(cell);
                            } else if (nullcell < 7 && cellIndex < 43 && cellIndex > 14) {
                                nullcell = 0;
                            } else if (nullcell == 7 && cellIndex > 14) {
                                nullcell = 0;
                ...
                //remove null cell
                if($(calendar.find(">div")[16]).hasClass("outday")){
                	 for (var i = 16; i >= 10; i--) {
                	 	$(calendar.find(">div")[i]).remove(".outday");
                	 }
                }else if($(calendar.find(">div")[45])){
                	for (var i = 51; i >= 45; i--) {
                	 	$(calendar.find(">div")[i]).remove(".outday");
                	 }
                }
