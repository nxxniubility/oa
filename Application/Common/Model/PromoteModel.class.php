...
class PromoteModel extends SystemModel{
	protected $proidDb;
	protected $promoteDb;
	protected $pagesDb;
    public function _initialize(){
        $this->proidDb = M('proid');
        $this->promoteDb = M('promote');
        $this->pagesDb = M('pages');
        $this->channelDb = M('channel');
        $this->servicecodeDb = M('servicecode');
        $this->prolevDb = M('pro_lev');
        $this->setPagesDb = M('setpages');
    ...
    */ 
	public function editPromoteInfo($promote)
	{
		if ($promote['pro_lev_id']) {
			...
			$updatepromote = $this->where($promotes)->save($promote);
			if ($updatepromote === false) {
				return false;
			}
		}else{
    ...
    */
	...
	{
		$promote['status'] = 1;
		$proInfos = $this->promoteDb->where($promote)->select();
		if (!$proInfos) {
			return false;
		}
